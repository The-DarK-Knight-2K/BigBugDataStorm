...
\subsection{Methodology: Pseudo-Labelled CatBoost Demand Ceiling}

We selected a gradient-boosted tree model trained on a pseudo-labelled upper-tail
...
\begin{infobox}[Model Approach]
  ...
  CatBoost regressor using Gold-layer structural features. CatBoost was chosen
  after cross-validation outperformed LightGBM (CV RMSE 40.38 vs. 40.96).
\end{infobox}
...
\begin{tabular}{ll}
  ...
  \midrule
  Historical avg. monthly volume (12-month) & transactions\_history \\
  Peak-to-average ratio (demand volatility)  & transactions\_history \\
  Outlet type, channel class                 & outlet\_master \\
  Distributor seasonality index (Jan)        & distributor\_seasonality \\
  Holiday-adjusted trading days (Jan 2026)   & holiday\_list \\
  POI count \& diversity (500 m)             & Overpass API \\
  Nearest transit / school distance          & Overpass API \\
  Province-level market index                & Derived \\
  \bottomrule
\end{tabular}
\end{center}
