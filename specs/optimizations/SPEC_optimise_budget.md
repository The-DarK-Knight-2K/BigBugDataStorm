...
*   **Tier 2 (Medium Potential):** Promotional Discount (~1,200 LKR base). Expected uplift conversion: 10%.
...
*   **Constraints:** `Sum of Spend <= 5,000,000 LKR`. Spend per outlet must be either 0, or between 500 and 15,000 LKR (in multiples of 50). Only outlets with `province == 'Western'` can receive budget.

## 6. Budget Balancing
After the optimal tier boundaries are found, the script will execute a balancing loop on Tier 1 and Tier 2 allocations using 50 LKR multiples to ensure the sum reaches or approaches `<= 5,000,000 LKR`.

## 7. Outputs
Outputs are saved to the following directories:

### Saved in `outputs/`
1.  `bigbug_budget_allocations.csv`: Two columns (`Outlet_ID`, `Trade_Spend_Allocation_LKR`). Contains **only** Western Province outlets.
2.  `budget_diagnostics.csv`: Full diagnostics including calculated ROI, tier assignment, and projected profit.
3.  `roi_distribution.png`: Histogram visualization of the ROI score distribution with overlaid tier cutoffs.

### Saved in `data/Optimization/`
4.  `budget_features.parquet`: Parquet version of diagnostics.
