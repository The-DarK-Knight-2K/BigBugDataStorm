...
import { mockBudgetSummary, mockOutlets } from '@/lib/mockData';
...
export default function BudgetDashboard() {
  ...
  // Total WP budget spend aggregates from mock summary data
  const totalBudget = mockBudgetSummary.total_trade_spend_allocation_lkr;
  const expectedLift = mockBudgetSummary.projected_volume_uplift_litres;

  // Donut chart: Budget per Distributor
  const donutData = useMemo(() => {
    return mockBudgetSummary.summary_by_distributor.map((d, index) => ({
  ...
  const barData = useMemo(() => {
    return mockBudgetSummary.summary_by_distributor.map(d => ({
      name: d.distributor_id,
      litres: d.lift,
      spend: d.total_spend
    }));
  ...
  const allocationOutlets = useMemo(() => {
    const outletsWithBudget = mockOutlets.filter(o => o.has_budget_allocation);
    if (!selectedDistributor) return outletsWithBudget;
    return outletsWithBudget.filter(o => o.distributor_id === selectedDistributor);
  }, [selectedDistributor]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl relative overflow-hidden border-l-4 border-l-cyan-500">
          ...
          <span className="text-[10px] text-slate-400 mt-2 block font-mono">
            Across 6,842 optimized trade accounts
          </span>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl relative overflow-hidden border-l-4 border-l-emerald-500">
          ...
          <p className="text-3xl font-heading font-extrabold text-emerald-400 text-glow-emerald mt-2">
            {expectedLift.toLocaleString()} <span className="text-xs text-slate-400">L</span>
          </p>
          <span className="text-[10px] text-emerald-400 mt-2 block font-mono">
            Average LKR {Math.round(totalBudget / expectedLift).toFixed(1)} spend per liter lift
          </span>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4 flex flex-col justify-between">
          ...
          <div className="h-[250px] w-full flex items-center justify-center relative">
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-slate-900 border border-slate-800 p-2.5 rounded-lg shadow-xl text-[11px]">
                          ...
                          <p className="text-slate-300 mt-1">Allocation: <span className="font-bold font-mono text-white">LKR {data.value.toLocaleString()}</span></p>
            ...
            <div className="absolute flex flex-col items-center justify-center pointer-events-none">
              ...
              <span className="text-xl font-heading font-extrabold text-white mt-1.5 font-mono">5.0 M LKR</span>
            </div>
          </div>
        </div>
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-slate-900 border border-slate-800 p-2.5 rounded-lg shadow-xl text-[11px]">
                          <p className="font-bold text-white uppercase">{data.name}</p>
                          <p className="text-emerald-400 mt-1 font-bold">Projected Lift: <span className="font-mono">{data.litres.toLocaleString()} L</span></p>
                          <p className="text-slate-400">Total Spend LKR: <span className="font-mono">{data.spend.toLocaleString()}</span></p>
                        </div>
                      );
                    }
                    return null;
        ...
        <div className="p-6 border-b border-slate-800 flex flex-wrap gap-4 justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="DIST_W_01">DIST_W_01</option>
              <option value="DIST_W_02">DIST_W_02</option>
              <option value="DIST_W_03">DIST_W_03</option>
            </select>
          </div>
        </div>
                ...
                allocationOutlets.map((outlet) => (
                  <tr key={outlet.outlet_id} className="hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-300 font-medium">
                      {getActivityType(outlet.allocation_tier)}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Link
