...
import { mockOutlets } from '@/lib/mockData';
...
export default function Dashboard() {
  ...
  // Extract unique filter options from mock dataset dynamically
  const filterOptions = useMemo(() => {
    const provinces = Array.from(new Set(mockOutlets.map(o => o.province)));
    const distributors = Array.from(new Set(mockOutlets.map(o => o.distributor_id)));
    const types = Array.from(new Set(mockOutlets.map(o => o.outlet_type)));
    const tiers = Array.from(new Set(mockOutlets.map(o => o.allocation_tier))).filter(t => t !== 'none');
  ...
  const stats = useMemo(() => {
    ...
    const totalVolume = filteredOutlets.reduce((sum, o) => sum + (o.Maximum_Monthly_Liters || 0), 0);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border-l-4 border-l-emerald-500 relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
          ...
          <p className="text-3xl font-heading font-extrabold text-white text-glow-emerald mt-2">
            LKR {stats.totalBudget.toLocaleString()}
          </p>
          <span className="text-[10px] text-emerald-400 flex items-center gap-1 mt-1 font-mono">
          ...
          <div className="h-[450px]">
            <MapComponent 
              outlets={filteredOutlets.map(o => ({
                ...
                Maximum_Monthly_Liters: o.Maximum_Monthly_Liters,
                ...
                filteredOutlets.map((outlet) => (
                  <tr key={outlet.outlet_id} className="hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right text-white font-semibold font-mono">
                      {Math.round(outlet.Maximum_Monthly_Liters).toLocaleString()}
                    </td>
                    <td className="px-6 py-4 text-right text-slate-400 font-mono">
                      {Math.round(outlet.recent_3m_avg).toLocaleString()}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-extrabold tracking-wider ${
                        ...
                        {outlet.allocation_tier !== 'none' ? outlet.allocation_tier : 'NONE'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Link 
