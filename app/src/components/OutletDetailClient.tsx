...
export default function OutletDetailClient({ outlet }: { outlet: OutletDetail }) {
  ...
  const FEATURE_MAP: Record<string, string> = {
    ...
    "size_imputed": "Estimated Store Size",
    "seasonality_multiplier_jan_2026": "Jan 2026 Seasonality Multiplier"
  };

  // Feature Descriptions for SHAP tooltips
  const FEATURE_DESCRIPTIONS: Record<string, string> = {
    "trend_slope": "The short-term trend of sales over the last few months. A positive value indicates accelerating growth.",
    "hist_cv": "Measures how erratic this outlet's sales are month-to-month. Higher values mean unpredictable ordering patterns.",
    "yoy_growth_rate": "The percentage change in sales compared to the exact same period last year.",
    "jan_count": "The historical volume specifically generated during the January peak season.",
    "recent_3m_avg": "The average monthly volume ordered by this outlet over the last 90 days.",
    "hist_p90_monthly": "The highest monthly volume this outlet has ever historically achieved (90th percentile).",
    "active_months": "The total number of historical months this outlet has recorded at least one order.",
    "active_months_pct": "The percentage of months this outlet placed an order since becoming a customer. High % means a highly reliable buyer.",
    "consecutive_zero_months_max": "The longest period this outlet went without placing any orders. Indicates churn risk.",
    "months_since_last_order": "Time elapsed since the outlet's most recent purchase.",
    "outlet_size": "The physical floor space of the outlet, which limits maximum shelf capacity.",
    "size_imputed": "The physical floor space of the outlet, which limits maximum shelf capacity.",
    "cooler_count": "The number of company coolers currently deployed at this location.",
    "seasonality_multiplier_jan_2026": "The expected volume multiplier applied specifically for the upcoming January season.",
    "worship_gravity_score": "Foot traffic potential generated by proximity to temples, churches, or mosques.",
    "market_gravity_score": "Overall consumer density and commercial activity in the immediate surrounding area.",
    "transport_gravity_score": "Foot traffic potential generated by proximity to bus stands or train stations.",
    "hospitality_gravity_score": "Foot traffic potential driven by nearby tourism and hospitality venues.",
    "school_gravity_score": "Foot traffic potential driven by proximity to educational facilities.",
    "hospital_gravity_score": "Foot traffic potential driven by proximity to medical facilities.",
    "competitors_500m": "The density of direct competitor outlets within a very short walking distance.",
    "competitors_1km": "The density of competitor outlets in the surrounding 1km area.",
    "competitors_2km": "The density of competitor outlets in the broader neighborhood."
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl relative group hover:border-slate-700 transition-colors">
          <div className="flex items-center text-xs font-semibold uppercase tracking-wider text-slate-400">
            Max monthly potential <TooltipInfo content="Includes the impact of local seasonal trends." />
          </div>
        ...
        <div className="glass-panel p-6 rounded-2xl relative group hover:border-slate-700 transition-colors">
          <div className="flex items-center text-xs font-semibold uppercase tracking-wider text-slate-400">
            Location Traffic Score <TooltipInfo content="A score representing expected customer foot traffic based on the outlet's location and nearby points of interest." />
          </div>
              ...
              <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                <div className="flex justify-between items-center mb-1.5">
                  <div className="flex items-center text-slate-400 text-[10px] uppercase">
                    Utilization Ratio <TooltipInfo content="Current volume divided by the maximum physical storage capacity." />
                  </div>
            ...
            <div className="space-y-3 font-mono text-xs">
              <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 flex justify-between items-center">
                <div className="flex items-center text-slate-400">
                  Local Competitor Crowding <TooltipInfo content="An index measuring the density of competing outlets nearby. Higher scores indicate higher competition." />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-2">
                <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                  <div className="flex items-center text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                    True Demand Est. <TooltipInfo content="The total theoretical volume this outlet could sell based on local market size." />
                  </div>
                  <span className="text-sm font-extrabold text-white mt-1 block">{(outlet.tobit_latent_estimate || 0).toLocaleString()} L</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                  <div className="flex items-center text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                    Sales Likelihood <TooltipInfo content="The realistic volume we expect to capture after factoring in competition and historical performance." />
                  </div>
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        ...
                        const desc = FEATURE_DESCRIPTIONS[data.raw_feature] || 'Measures the impact of this feature on the predicted volume.';
                        return (
                          <div className="bg-slate-900 border border-slate-800 p-3 rounded-xl shadow-xl max-w-xs text-[11px]">
                            <p className="font-bold text-white uppercase text-xs">{data.name}</p>
                            <p className="text-slate-300 mt-1 mb-2 font-medium">Volume Impact: <span className={`font-bold font-mono ${data.val > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{data.val > 0 ? '+' : ''}{Number(data.val).toFixed(1)} L</span></p>
                            <hr className="border-slate-800/60 my-2" />
                            <p className="text-slate-400 italic leading-relaxed">{desc}</p>
                          </div>
                        );
                      }
                      return null;
